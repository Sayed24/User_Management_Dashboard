...
userForm.onsubmit = e => {
  ...
  if (!firstName.value) errFirstName.textContent = "Required", ok=false;
  else errFirstName.textContent = "";

  if (!lastName.value) errLastName.textContent = "Required", ok=false;
  else errLastName.textContent = "";

  if (!email.value.includes("@")) errEmail.textContent = "Invalid email", ok=false;
